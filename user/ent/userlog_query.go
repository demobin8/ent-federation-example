...
// Returns a *NotSingularError when exactly one UserLog entity is not found.
...
// Returns a *NotSingularError when exactly one UserLog ID is not found.
...
//		Scan(ctx, &v)
//
func (ulq *UserLogQuery) Select(field string, fields ...string) *UserLogSelect {
...
func (ulq *UserLogQuery) sqlQuery(ctx context.Context) *sql.Selector {
	...
	selector := builder.Select(t1.Columns(userlog.Columns...)...).From(t1)
	if ulq.sql != nil {
		selector = ulq.sql
		selector.Select(selector.Columns(userlog.Columns...)...)
	}
...
func (uls *UserLogSelect) sqlQuery() sql.Querier {
	selector := uls.sql

...
type UserQuery struct {
	...
	withUserOaInfo *UserOaInfoQuery
	withUserOpInfo *UserOpInfoQuery
...
// QueryUserLogs chains the current query on the "userLogs" edge.
func (uq *UserQuery) QueryUserLogs() *UserLogQuery {
...
// QueryUserOaInfo chains the current query on the "userOaInfo" edge.
func (uq *UserQuery) QueryUserOaInfo() *UserOaInfoQuery {
	query := &UserOaInfoQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// QueryUserOpInfo chains the current query on the "userOpInfo" edge.
func (uq *UserQuery) QueryUserOpInfo() *UserOpInfoQuery {
	query := &UserOpInfoQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withUserLogs:   uq.withUserLogs.Clone(),
		withUserOaInfo: uq.withUserOaInfo.Clone(),
		withUserOpInfo: uq.withUserOpInfo.Clone(),
...
// the "userLogs" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithUserLogs(opts ...func(*UserLogQuery)) *UserQuery {
	query := &UserLogQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withUserLogs = query
	return uq
}

// WithUserOaInfo tells the query-builder to eager-load the nodes that are connected to
// the "userOaInfo" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithUserOaInfo(opts ...func(*UserOaInfoQuery)) *UserQuery {
	query := &UserOaInfoQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withUserOaInfo = query
	return uq
}

// WithUserOpInfo tells the query-builder to eager-load the nodes that are connected to
// the "userOpInfo" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithUserOpInfo(opts ...func(*UserOpInfoQuery)) *UserQuery {
	query := &UserOpInfoQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withUserOpInfo = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	var (
		...
		loadedTypes = [3]bool{
			uq.withUserLogs != nil,
			uq.withUserOaInfo != nil,
			uq.withUserOpInfo != nil,
		}
	)
	...
	if query := uq.withUserLogs; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*User)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.UserLogs = []*UserLog{}
		}
		query.withFKs = true
		query.Where(predicate.UserLog(func(s *sql.Selector) {
			s.Where(sql.InValues(user.UserLogsColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.user_user_logs
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_user_logs" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_user_logs" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.UserLogs = append(node.Edges.UserLogs, n)
		}
	}

	if query := uq.withUserOaInfo; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*User)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.UserOaInfo = []*UserOaInfo{}
		}
		query.withFKs = true
		query.Where(predicate.UserOaInfo(func(s *sql.Selector) {
			s.Where(sql.InValues(user.UserOaInfoColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.user_user_oa_info
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_user_oa_info" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_user_oa_info" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.UserOaInfo = append(node.Edges.UserOaInfo, n)
		}
	}

	if query := uq.withUserOpInfo; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*User)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.UserOpInfo = []*UserOpInfo{}
		}
		query.withFKs = true
		query.Where(predicate.UserOpInfo(func(s *sql.Selector) {
			s.Where(sql.InValues(user.UserOpInfoColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.user_user_op_info
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_user_op_info" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_user_op_info" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.UserOpInfo = append(node.Edges.UserOpInfo, n)
		}
	}

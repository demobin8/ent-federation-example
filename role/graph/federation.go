...
func (ec *executionContext) __resolve_entities(ctx context.Context, representations []map[string]interface{}) ([]fedruntime.Entity, error) {
	...
	for _, rep := range representations {
		...
		case "Permission":
			...
			entity, err := ec.resolvers.Entity().FindPermissionByID(ctx,
				id0)
			if err != nil {
				return nil, err
			}
		...
		case "Role":
			...
			entity, err := ec.resolvers.Entity().FindRoleByID(ctx,
				id0)
			if err != nil {
				return nil, err
			}
		...
		case "UserRole":
			...
			id1, err := ec.unmarshalNInt2int(ctx, rep["roleId"])
			if err != nil {
				return nil, errors.New(fmt.Sprintf("Field %s undefined in schema.", "roleId"))
			}

			entity, err := ec.resolvers.Entity().FindUserRoleByUserIDAndRoleID(ctx,
				id0, id1)
			if err != nil {
				return nil, err
			}

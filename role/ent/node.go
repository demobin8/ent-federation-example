...
func (pe *Permission) Node(ctx context.Context) (node *Node, err error) {
	...
	if buf, err = json.Marshal(pe.Code); err != nil {
		return nil, err
	}
	node.Fields[1] = &Field{
		Type:  "string",
	...
	if buf, err = json.Marshal(pe.Remark); err != nil {
		return nil, err
	}
	node.Fields[2] = &Field{
		Type:  "string",
	...
	node.Edges[0].IDs, err = pe.QueryRole().
		Select(role.FieldID).
		Ints(ctx)
	if err != nil {
		return nil, err
...
func (r *Role) Node(ctx context.Context) (node *Node, err error) {
	...
	if buf, err = json.Marshal(r.Remark); err != nil {
		return nil, err
	}
	node.Fields[1] = &Field{
		Type:  "string",
	...
	node.Edges[0].IDs, err = r.QueryPermissions().
...
func (c *Client) noder(ctx context.Context, table string, id int) (Noder, error) {
	...
	case permission.Table:
		n, err := c.Permission.Query().
			Where(permission.ID(id)).
			CollectFields(ctx, "Permission").
			Only(ctx)
		if err != nil {
			return nil, err
		}
	...
	case role.Table:
		...
		return n, nil
	case userrole.Table:
		n, err := c.UserRole.Query().
			Where(userrole.ID(id)).
			CollectFields(ctx, "UserRole").
			Only(ctx)
		if err != nil {
			return nil, err
		}
